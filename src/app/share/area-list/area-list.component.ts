...
import { getCitiesByProvince, getAreaByCity, getProvinces} from '../../utils/area.util';
...
export class AreaListComponent implements OnInit, ControlValueAccessor, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.districes$ = Observable.combineLatest(province$, city$, (p: string, c: string) => getAreaByCity(p, c));
  }
  ngOnDestroy() {
    if (this.sub) {
